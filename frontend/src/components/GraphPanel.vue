...
const renderGraph = () => {
  ...
  // Process edge data, calculate edge count and index for each node pair
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Flip curvature if edge direction is reversed from normalized direction
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150, increase by 50 for each additional edge
  ...
  const getLinkPath = (d) => {
    ...
    // More edges result in larger offset ratio
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Only gradually stop simulation if actually dragged
        if (d._isDragging) {
