...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### Sub-questions\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of reason (excluding closing summary paragraph)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer available, copy as default display when not placeholder text
          ...
          // No platform markers (very old format), treat entire text as answer
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No current key memory available'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No current active memory available'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" (new backend format)
      // 2. "1. " or "\n1. " (number+dot, legacy format compatible)
      let matches = []
      let match

      // Prefer "QuestionX:" format first
      const enPattern = /(?:^|[\r\n]+)Question(\d+)[：:]\s*/g
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 heading (## xxx) since section title is already displayed in outer layer
  ...
  // Clean up <p><br> immediately followed by block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> between block-level closing tags and paragraph opening tags
...
const extractFinalContent = (response) => {
  ...
  // Legacy Chinese "Final Answer:" marker removed (backend is now English-only)
