    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context to automatically extract entities and relations from the knowledge graph, initialize simulated individuals, and assign unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, active time periods, posting frequency, and event triggers based on simulation requirements and reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, automatically generates initial activation events and hot topics to guide the simulation world's initial state
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish auto-plans simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real time</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first-time runs, we strongly recommend switching to ‘Custom’ mode and reducing rounds for a quick preview with lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
// State
const phase = ref(0) // 0: init, 1: generate profiles, 2: generate config, 3: complete
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Entering config generation stage, start polling config
    if (!configTimer) {
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected existing preparation, reusing data')
