...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost nobody at dawn
...
- Target user group is Chinese users, must follow Beijing Time daily schedule
- Almost no activity from 0-5 AM (activity multiplier 0.05)
- Gradually active from 6-8 AM (activity multiplier 0.4)
- Moderately active during work hours 9 AM-6 PM (activity multiplier 0.7)
- Peak period from 7-10 PM (activity multiplier 1.5)
- Decreasing activity after 11 PM (activity multiplier 0.5)
- General pattern: low activity at dawn, increasing in morning, moderate during work, evening peak
- **Important**: The following example values are for reference only. You need to adjust specific time periods based on event nature and participant group characteristics.
  - E.g.: Student groups may peak at 9-11 PM; media active all day; official institutions only during work hours
  - E.g.: Breaking news may cause late-night discussions, off_peak_hours can be shortened accordingly
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration must follow Chinese daily schedule patterns."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration result, validating that agents_per_hour values do not exceed total agent count"""
        # Get raw values
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            # 3. If still no match found, use the agent with highest influence
...
- **Schedule follows Chinese daily patterns**: Almost no activity from 0-5 AM, most active from 7-10 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations must follow Chinese daily schedule patterns."
