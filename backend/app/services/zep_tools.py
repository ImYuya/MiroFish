...
Encapsulates graph search, node retrieval, edge search, and other tools for Report Agent.

Core search tools (optimized):
1. InsightForge (deep insight search) - Most powerful hybrid search, auto-generates sub-queries for multi-dimensional retrieval
2. PanoramaSearch (broad search) - Retrieves full view, including expired content
...
class EdgeInfo:
    ...
    def is_expired(self) -> bool:
        """Whether this edge is expired"""
        return self.expired_at is not None

    @property
    def is_invalid(self) -> bool:
        """Whether this edge is invalidated"""
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## In-Depth Predictive Analysis",
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Panoramic Search Results (Future Panorama View)",
            f"Query: {self.query}",
            f"\n### Statistics",
        ...
        # Entities (full output, no truncation)
        if self.all_nodes:
            text_parts.append(f"\n### [Entities Involved]")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out garbage content containing question numbers
                skip = False
                for d in '123456789':
                    if f'\u95ee\u9898{d}' in clean_quote or f'Question {d}' in clean_quote or f'Question{d}' in clean_quote:
                        skip = True
                        break
                if skip:
                    continue
                ...
                if len(clean_quote) > 150:
                    # Try Chinese period, then English period
...
class ZepToolsService:
    ...
    [Core Search Tools - Optimized]
    1. insight_forge - Deep insight search (most powerful, auto-generates sub-queries, multi-dimensional retrieval)
    2. panorama_search - Broad search (retrieves full view, including expired content)
    3. quick_search - Simple search (fast search)
    4. interview_agents - Deep interview (interviews simulated Agents, obtains multi-perspective views)

    [Basic Tools]
    - search_graph - Graph semantic search
    - get_all_nodes - Retrieve all nodes in the graph
    - get_all_edges - Retrieve all edges in the graph (with temporal info)
    - get_node_detail - Retrieve detailed node information
    - get_node_edges - Retrieve edges related to a node
    - get_entities_by_type - Retrieve entities by type
    - get_entity_summary - Retrieve relationship summary for an entity
    """
    ...
    def search_graph(
        ...
        Uses LocalGraphStore's hybrid search (semantic + keyword).
    ...
    def _local_search(
        ...
        """
        Local keyword matching search (fallback for semantic search)
        """
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """Retrieve all edges in the graph (with temporal info)"""
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        logger.info(f"Retrieving node detail: {node_uuid[:8]}...")
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        try:
            edges = self.store.get_node_edges(node_uuid, graph_id)
    ...
    def get_graph_statistics(self, graph_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"Retrieving statistics for graph {graph_id}...")
    ...
    def get_simulation_context(
        ...
        """Retrieve simulation-related context information"""
    ...
    def insight_forge(
        ...
        The most powerful hybrid search function, auto-decomposes queries for multi-dimensional retrieval.
        """
        ...
        # Retrieve details for all related entities
        entity_insights = []
        node_map = {}
        ...
        for uuid in list(entity_uuids):
            ...
            try:
                ...
                logger.debug(f"Failed to retrieve node {uuid}: {e}")
                continue
...
Return a JSON-formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        Retrieves full panoramic view, including all related content and historical/expired information.
        """
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question in order, starting with 'Question X:' (X = question number)\n"
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                clean_text = re.sub(r'(?:问题|Question\s*)\d+[：:.\s]\s*', '', clean_text)
