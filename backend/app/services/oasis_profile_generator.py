...
Converts entities from the graph into Agent Profile format required by the OASIS simulation platform

Optimization improvements:
1. Call LocalGraphStore retrieval to enrich node information
...
class OasisProfileGenerator:
    ...
    # Group/institutional entity types (require generating representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        zep_api_key: Optional[str] = None,  # kept for backward compat, unused
        ...
        # LocalGraphStore for enrichment search
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use LocalGraphStore search to obtain rich information related to the entity

        Searches edges and nodes separately, then merges results.
        ...
        if not self.graph_id:
            logger.debug(f"Skipping search: graph_id not set")
            return results
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximally restoring existing real-world conditions.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Background (important experiences, association with events, social relationships)
   ...
   - Positions and viewpoints (attitude toward topics, content that may provoke or move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of the persona, introduce this individual's association with events, and their existing actions and reactions in the events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for the institution/group entity, maximally restoring existing real-world conditions.
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   - Institutional basic information (official name, institutional nature, founding background, main functions)
   ...
   - Institutional memories (important part of the institutional persona, introduce this institution's association with events, and its existing actions and reactions in the events)
...
- Institutional account communications must align with its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        completed_count = [0]  # Use list to allow modification within closure
        lock = Lock()
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        """
        Normalize the gender field to English format required by OASIS
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Must include the user_id field, which is critical for OASIS agent_graph.get_agent() matching!
