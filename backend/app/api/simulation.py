...
def create_simulation():
    ...
    Note: Parameters such as max_rounds are intelligently generated by LLM; no manual setup required.
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files list (excluding scripts, which are located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # The following statuses all indicate that preparation is done:
        # - ready: preparation complete, ready to run
        # - preparing: if config_generated=True, it means preparation finished
...
def prepare_simulation():
    ...
    - If already prepared, returns the existing result directly
    ...
    try:
        ...
        # Get necessary info from the project
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Skip edge info to speed up
            )
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns a simulation list
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - generation_reasoning: LLM's reasoning for the generated config
    """
    try:
        manager = SimulationManager()
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": "Simulation config not found. Please call the /prepare endpoint first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Config file not found. Please call the /prepare endpoint first"
            }), 404
...
def start_simulation():
    ...
    About the force parameter:
        ...
        - Suitable for scenarios that require re-running the simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) are updated to the Zep graph in real-time
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is currently running. Please call the /stop endpoint first, or use force=true to force restart"
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation not ready, current status: {state.status.value}. Please call the /prepare endpoint first"
                }), 400
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (in command-waiting mode after completing simulation rounds).
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been shut down. Please ensure the simulation has completed and entered command-waiting mode."
            }), 400
...
def close_simulation_env():
    ...
    Sends a shutdown command to the simulation for a graceful exit from command-waiting mode.
