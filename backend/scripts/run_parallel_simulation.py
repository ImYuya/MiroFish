...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally leave max_tokens unset, letting the model decide)"""
...
# Available Twitter actions (excludes INTERVIEW, which can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (excludes INTERVIEW, which can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If a boost LLM is configured, parallel simulations can use different API providers
...
async def run_twitter_simulation(
    ...
    log_info(f"Simulation loop completed! Duration: {elapsed:.1f}s, total actions: {total_actions}")
...
async def main():
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop completed! Total duration: {total_elapsed:.1f}s")
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (giving loop a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
